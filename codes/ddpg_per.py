...
if __name__ == "__main__":
    ...
    env = TradingEnv(continuous_action_flag=True, sabr_flag=False, dg_random_seed=1, init_ttm=20, trade_freq=1, spread=0.01, num_contract=1, num_sim=10002)
    ...
    history = ddpg.train(1001)
    ddpg.save_history(history, "ddpg1d.csv")
    

    # setup for testing; use another instance for testing
    env_test = TradingEnv(continuous_action_flag=True, sabr_flag=True, dg_random_seed=2, init_ttm=20, trade_freq=1, spread=0.01, num_contract=1, num_sim=10001)
    ddpg_test = DDPG(env_test)
    ddpg_test.load()
    #history2 = ddpg_test.train(500)
