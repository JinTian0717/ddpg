...
class TradingEnv(gym.Env):
    ...
    def __init__(self, cash_flow_flag=0, dg_random_seed=1, num_sim=500002, sabr_flag = False,
        ...
        if continuous_action_flag:
            ...
            #self.obs_space = spaces.Box(low=np.array([0]), high=np.array([num_contract * 100]), dtype=np.float32)
        else:
    ...
    def step_profit_loss(self, action):
        ...
        if self.t == self.num_period - 1:
            ...
            reward -= 0.1*(reward**2)
            reward -= np.abs(current_position - position) * current_price * self.spread
        else:
            done = False
